...
IMAGE_FALLBACK_PLATFORMS = {"instagram", "pinterest"}
...
class Downloader:
    ...
    def _download_image_fallback(self, url: str, file_id: str) -> str | None:
        """
        وقتی yt-dlp نتونه یه پست رو دانلود کنه (معمولا چون فقط عکسه، نه ویدیو)،
        مستقیم صفحه رو fetch می‌کنیم و تگ og:image رو استخراج می‌کنیم.
        اینستاگرام و پینترست همیشه این تگ رو توی HTML صفحه می‌ذارن.
        """
        try:
        ...
        match = _OG_IMAGE_RE.search(page.text)
        if not match:
            return None
        ...
        try:
            img_resp = requests.get(image_url, headers=_HTTP_HEADERS, timeout=20, stream=True)
    ...
    def _download_sync(self, url: str, platform: str) -> tuple[str, str]:
        ...
        except (yt_dlp.utils.DownloadError, DownloadError) as e:
            # اگه ویدیو نبود احتمالا پست عکسیه - سعی می‌کنیم با روش جایگزین بگیریمش
            if platform in IMAGE_FALLBACK_PLATFORMS:
                fallback_path = self._download_image_fallback(url, file_id)
                if fallback_path:
                    return fallback_path, "photo"

            raw = str(e) if isinstance(e, yt_dlp.utils.DownloadError) else str(e)
            raise DownloadError(self._friendly_message(raw))
        except Exception:
